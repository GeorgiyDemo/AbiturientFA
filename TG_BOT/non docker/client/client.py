            ...
            if update_request != []:
                for update in update_request:
                    if int(update["changed_from"]) > int(update["changed_to"]):
                        ...
                        viewflag = "⬇️"
                    bot.send_message(chat_id=update["tid"],
                                     text=viewflag + " Изменение места с " + update["changed_from"] + " на " + update[
                                         "changed_to"] + " на направлении " + update["wayname"])
        except:
            continue
        time.sleep(60)
...
def handler(bot):
    ...
    for update in bot.get_updates(offset=update_id, timeout=10):
        ...
        if update.message.text == "/start":
            ...
            r = requests.post("http://127.0.0.1:5000/adduser",
